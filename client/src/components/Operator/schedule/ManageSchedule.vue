        ...
        <v-form ref="schedule_form">
                  ...
                  <v-row>
                    <v-col>
                      <v-menu
                        ...
                        offset-y
                        :disabled="!editMode && !!TempScheduleIdState"
                        max-width="290px"
                        min-width="290px"
                      >
          ...
          <v-row>
            <v-col>
              ...
              <v-btn
                v-if="
                  TempScheduleIdState && !editMode && !OrdersHistoryModeState
                "
                @click="editMode = true"
                >Edit</v-btn
              >
              <v-btn
                ...
                @click="editPickupSchedule"
                >Save</v-btn
              >
              <v-btn v-if="OrdersHistoryModeState" @click="showManageSchedule"
                >CLOSE</v-btn
              >
...
export default {
  ...
  methods: {
    ...
    checkDateTime(v) {
      if (
        this.getCurrentTime() <= v &&
        this.pickupTime == this.getCurrentDate()
      ) {
        return "Invalid";
      } else {
        return true;
      ...
      if (this.$refs.schedule_form.validate()) {
        try {
          ...
          if (value.isConfirmed) {
            await ScheduleService.editPickupSchedule(
              this.TempScheduleIdState,
              this.createScheduleFormData()
            );
            this.editMode = false;
            this.$swal({
          ...
          console.log(err);
          this.$swal({
            title: "Update Schedule Fail, Please try again",
            icon: "error",
            timer: 1000,
            showConfirmButton: false,
            showCancelButton: false,
    ...
    async createSchedule() {
      try {
        if (this.$refs.schedule_form.validate()) {
          let result = await ScheduleService.createSchedule(
            this.createScheduleFormData()
          );
        ...
        console.log(err);
        this.$swal({
          title: "Create Schedule Fail, Please try again",
          icon: "error",
          timer: 1000,
          showConfirmButton: false,
          showCancelButton: false,
    ...
    async getSchedule() {
      try {
        let result = await ScheduleService.getScheduleById(
          this.TempScheduleIdState
        );
        let data = result.data[0];
        if (data.arrived_datetime) {
          this.arrived_datetime = `${this.getTime(
            data.arrived_datetime
          )} ${this.getDate(data.arrived_datetime)}`;
        }
